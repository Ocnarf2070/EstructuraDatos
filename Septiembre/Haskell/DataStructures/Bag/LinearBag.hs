...
insert x n@(Node (y, oy) s) 
	| x > y = Node (y,oy) (insert x s)
	| x == y = Node (y, oy+1) s 
	| otherwise = Node (x,1) n
...
union s Empty = s
union Empty s =  s
union b@(Node (x, ox) s) b'@(Node (y, oy) t) = foldBag f b b'
	where
		f z 1 bg= insert z bg
		f z oz bg= insert z (f z (oz-1) bg)

intersection :: Ord a => Bag a -> Bag a -> Bag a
intersection s Empty = Empty
intersection Empty s = Empty
intersection b@(Node (x, ox) s) b'@(Node (y, oy) t) 
	| x == y = Node (x, (min ox oy)) (intersection s t)
	| x < y = intersection s b'
	| otherwise = intersection b t


difference :: Ord a => Bag a -> Bag a -> Bag a
difference s Empty = s
difference Empty s = Empty
difference b@(Node (x, ox) s) b'@(Node (y, oy) t) = foldBag f b b'
	where
		f z 1 bg = delete z bg
		f z oz bg = delete z (f z (oz - 1) bg)
