...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x (sz + sz') mayor (merge menor1 menor2)
 | otherwise       = Node x' (sz + sz') mayor' (merge menor1' menor2')
 where
 (mayor, menor1, menor2) = ordenarArbol lh rh h'
 (mayor', menor1', menor2') = ordenarArbol lh' rh' h
 
ordenarArbol a b c
	| size b > size a = ordenarArbol b a c
	| size c > size a = ordenarArbol c b a
	| otherwise = (a, b, c)
----------------------------------------------------------
-- ^^^^^^^^^^^^^^-- SOLO TOCAR ARRIBA ^^^^^^^^^^^ ---------
----------------------------------------------------------
