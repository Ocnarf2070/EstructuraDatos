...
isAVL Empty = True
isAVL (Node x h lt rt)  = forAll (<x) lt && forAll (>x) rt && abs (height lt - height rt) < 2 && isAVL lt && isAVL rt
    where
...
minim (Node k _ Empty rt) = k
minim (Node k _ lt rt) = minim lt

maxim :: AVL a -> a
maxim Empty = error "maxim on empty tree"
maxim (Node k _ Empty rt) = k
maxim (Node k _ lt rt) = maxim lt
