...
dijkstraAux g vs rs d = dijkstraAux g (v':vs) (delete v' rs) (D.insert v' (p', cs') d)
    ...
    caminos = zip (D.keys d) (D.values d)
    -- extender los caminos del diccionario con aristas hasta nodo en rs
    extensiones = [(d, (p+p', cs++[d])) | (o, (p,cs)) <- caminos, (d,p') <- successors g o, d `elem` rs]
    --- seleccionar la extensión de menor coste
    (v', (p', cs')) = minimumBy comparePares extensiones

comparePares (v, (p, cs)) (v', (p', cs')) = if p < p' then LT else GT


shortestPaths :: Ord a => WeightedGraph a Int -> a -> [Path a]
shortestPaths g v = map (\x@(_,c) -> c) (D.values $ dijkstra g v)
