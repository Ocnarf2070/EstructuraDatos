...
levelsAux q 
  | isEmpty q = []
  | otherwise = (aux q) ++ levelsAux (foldl (flip enqueue) empty (resto q))
	where
		aux queue = if isEmpty queue then [] else (fst $ f queue) : aux (dequeue queue)
		resto queue = if isEmpty queue then [] else (snd $ f queue) ++ resto (dequeue queue)
		f q = (x',ts') where (Node x' ts') = first q
-- Encola los elementos de la lista xs (de cabeza a cola) en la cola queue
-- siempre que no sean arboles vacios   
enqueueAll queue xs = foldl (flip enqueue) queue xs
