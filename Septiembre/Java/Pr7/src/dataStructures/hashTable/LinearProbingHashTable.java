 ...
 */public class LinearProbingHashTable<K,V> implements HashTable<K,V>{
	...
	private int searchIdx(K key) {
		// to be completed
		int h = hash(key);
		while(keys[h]!=null && !keys[h].equals(key))h=(h+1)%keys.length;
		return h;
	}
	...
	public void delete(K key) {
		int h = searchIdx(key);
		if(keys[h]==null);// throw new NoSuchElementException(key+" is not in the table");
		else {
			keys[h]=null;
			values[h]=null;
			size--;
	...
	private class TableIter {
		...
		public void advance() {
			...
			if(keys[nextIdx]==null) {
				while(keys[nextIdx]==null)nextIdx = (nextIdx+1)%keys.length;
			}
			visited++;
			// to be completed
		}
