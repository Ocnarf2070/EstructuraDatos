...
public class DictionaryUtilities  {
	...
	Dictionary < V,Set<K> > inverseDictionary ( Dictionary<K,V> dicKv) {
		...
		for( Tuple2<K,V> kvTuple : dicKv.keysValues()){
			...
			if(dicVk.isDefinedAt(value)) set = dicVk.valueOf(value);
			set.insert(key);
			dicVk.insert(value, set);
	...
	Set<K> keysWithMaximaValue ( Dictionary<K,V> dicKv) {
		...
		for(Tuple2<K, V> tuple : dicKv.keysValues()) {
			K key = tuple._1();
			V value = tuple._2();
			if(maxV == null) {
				maxV = value;
				maxima.insert(key);
			}else if(maxV.equals(value)) {
				maxima.insert(key);
			}else if(maxV.compareTo(value)<0) {
				Iterator<K> it = maxima.iterator();
				while(it.hasNext())maxima.delete(it.next());
				maxV=value;
				maxima.insert(key);
			}
		}
