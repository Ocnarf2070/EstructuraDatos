	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < lineas.length; i++) {
			for(int j = 0; j < lineas[i].length(); j++) {
				if((lineas[i].charAt(j)>='a'&&lineas[i].charAt(j)<='z')||(lineas[i].charAt(j)>='A'&& lineas[i].charAt(j)<='Z')) {
					char character = lineas[i].charAt(j);
					Dictionary<Integer, Set<Integer>> ubicacion = new AVLDictionary<>();
					Set<Integer> set = new AVLSet<>();
					if(contador.isDefinedAt(character))ubicacion = contador.valueOf(character);
					if(ubicacion.isDefinedAt(i)) set = ubicacion.valueOf(i);
					set.insert(j);
					ubicacion.insert(i, set);
					contador.insert(character, ubicacion);
				}
