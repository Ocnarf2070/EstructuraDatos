...
weights xs = aux xs D.empty
	where
	aux [] dic = dic
	aux (x:xs) dic= D.insert x (1 + cont x xs) (aux xs dic)
		where
		cont x [] = 0
		cont x (y:ys) = if x == y then 1 + cont x ys else cont x ys
...
huffmanLeaves xs = aux PQ.empty (D.keysAndValues $ weights xs)
	where
	aux queue [] = queue
	aux queue (x@(c,i):xs) = PQ.enqueue (WLeaf c i) (aux queue xs)
...
huffmanTree xs 
	| PQ.isEmpty (PQ.dequeue queue) = error "huffmanTree: the string must have at least two different symbols"
	| otherwise = aux queue
	where
	queue = huffmanLeaves xs
	aux e1e2q 
		| PQ.isEmpty e2q = elem1
		| otherwise = aux $ PQ.enqueue (WNode elem1 elem2 sz) q
		where 
		elem1 = PQ.first e1e2q
		e2q = PQ.dequeue e1e2q 
		elem2 = PQ.first e2q
		q = PQ.dequeue e2q
		sz = weight elem1 + weight elem2
...
joinDics dic1 dic2
	| D.isEmpty dic1 = dic2
	| D.isEmpty dic2 = dic1
	| otherwise = aux dic1 (D.keysAndValues dic2)
		where
		aux dic [] = dic
		aux dic	(x@(c,i):xs)
			| not $ D.isDefinedAt c dic = aux (D.insert c i dic) xs
			| otherwise = error "The dictionaries must be disjointed"
...
prefixWith c dic = aux c dic (D.keys dic)
	where
	aux car d [] = d
	aux car d (k:ks) = aux car (D.updateOrInsert k (car :) v d) ks where (Just v) = D.valueOf k d
...
huffmanCode (WLeaf c _) = D.insert c ([]::[Integer]) D.empty
...
encode pal dic = foldr (++) ([]::[Integer]) $! map (codCar dic) pal
	where 
	codCar dic car = v where(Just v) = D.valueOf car dic
...
takeSymbol xs (WLeaf c _) = (c,xs)
takeSymbol [] (WNode lt rt _) = error "Codex not supported"
takeSymbol (x:xs) (WNode lt rt _)
	| x == 0 = takeSymbol xs lt
	| otherwise = takeSymbol xs rt
...
decode xs tree = aux xs tree
	where
		aux [] tree = []
		aux xs tree = c : aux ys tree
			where
			(c,ys) = takeSymbol xs tree
