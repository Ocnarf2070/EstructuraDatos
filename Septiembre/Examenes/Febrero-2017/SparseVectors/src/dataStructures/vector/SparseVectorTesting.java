...
public class SparseVectorTesting {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("testing SparseVector clone");
        {
        	 String message = "abbcccddddefgggh";
             SparseVector<Character> v = new SparseVector<>(4, INITIAL_ELEM);
             for (int i = 0; i < message.length(); i++) {
             ...
             check(message.length() == v.size(), "vector has wrong size");
             SparseVector<Character> clone = v.clone();
             check(v!=clone, "clone shoud not share the same memory");
        }
        System.out.println("DONE!\n");
        
        System.out.println("testing SparseVector depthOf");
        {
        	int a [] = {0,0,0,0,0,0,0,0};
        	SparseVector<Character> v = new SparseVector<Character>(3, 'a');
        	for (int i = 0; i < a.length; i++) {
        		check(a[i] == v.depthOf(i), "error on depthOf");
        	}
        }
        {
        	int a [] = {3,3,3,3,3,3,3,3};
        	String message = "abcdefgh";
            SparseVector<Character> v = new SparseVector<>((int)(Math.log(message.length())/Math.log(2)), INITIAL_ELEM);
            for (int i = 0; i < message.length(); i++) {
                v.set(i, message.charAt(i));
            }
            for (int i = 0; i < a.length; i++) {
        ...
        {
        	int a [] = {2,2,3,3,1,1,1,1};
        	String message = "aabcdddd";
            SparseVector<Character> v = new SparseVector<>((int)(Math.log(message.length())/Math.log(2)), INITIAL_ELEM);
            for (int i = 0; i < message.length(); i++) {
                v.set(i, message.charAt(i));
            }
