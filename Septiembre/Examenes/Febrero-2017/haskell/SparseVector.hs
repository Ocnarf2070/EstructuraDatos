...
vector n x = if (n<0) then error "negative vector not suported" else V (2^n) (Unif x) 
...
get i (V s t)
	| i >= 0 && i < s = aux i t 0 s
	|otherwise = error "index not valide"
	where
	aux ind (Unif a) _ _ = a
	aux ind (Node lt rt) init last
		| media > ind = aux ind lt init media
		| otherwise = aux ind rt media last
		where
		media = div (init + last) 2
...
set i c (V s t) 
	| i >= 0 && i < s = V s $!aux c i t 0 s
	|otherwise = error "index not valide"
	where
	aux cam ind (Unif a) init last 
		| ver == 1 = Unif cam
		| ver == 2 = if mod ind 2 == 0 then simplify (Unif cam) (Unif a) else simplify (Unif a) (Unif cam)
		| ver > 2 = aux cam ind (Node (Unif a) (Unif a)) init last
		
		where
		ver = last - init
	aux cam ind (Node lt rt) init last
		| media > ind = simplify (aux cam ind lt init media) rt
		| otherwise = simplify lt $!aux cam ind rt media last
		where
		media = div (init + last) 2
...
mapVector f (V s t) = V s $ map' f t
	where
	map' f (Unif a) = Unif (f a)
	map' f (Node lt rt) = simplify (map' f lt) (map' f rt)

-------------------------------------------------------------------------------
-- | Exercise h.
filterVector f (V s t) = filter' f t s
	where
	filter' f (Unif a) size = if f a then aux a size else []
	filter' f (Node lt rt) size = filter' f lt (div size 2) ++ filter' f rt (div size 2)
	aux x 0 = []
	aux x n = x : aux x (n-1)
...
depthOf ind (V s t) = depth 0 ind t 0 s
	where
	depth n i (Unif _) init last = n
	depth n ind (Node lt rt) init last
		| media > ind = depth (n+1) ind lt init media
		| otherwise = depth (n+1) ind rt media last
		where
		media = div (init + last) 2
