...
public class BinaryHeap<T extends Comparable<? super T>> implements Heap<T> {
   ...
   * Capacity is automatically increased when needed.
	...
	public BinaryHeap(BinaryHeap<T> h) {
		elements = Arrays.copyOf(h.elements,elements.length);
		size = h.size;
	}	
	...
	private int parent(int idx) { //returns index for parent of node with index idx
		return (idx-1) / 2;
	}
	
	private int leftChild(int idx) { //returns index for left child of node with index idx
		return 2*idx+1;
	}

	private int rightChild(int idx) { //returns index for right child of node with index idx
