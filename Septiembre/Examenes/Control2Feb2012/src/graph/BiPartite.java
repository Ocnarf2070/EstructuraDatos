	...
	public BiPartite(Graph<V> graph) {
		...
		while (!stack.isEmpty()&&isBiColored) {
			...
			if(dict.valueOf(vColor.first())==null ) {
				...
				for(V sec : graph.successors(vColor.first())) {
					Pair <V,Color> aux = new Pair<>(sec, nextColor(vColor.second()));
					stack.push(aux);
				}
			}
			else if(!dict.valueOf(vColor.first()).equals(vColor.second()))isBiColored=false;
