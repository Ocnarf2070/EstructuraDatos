...
addEdges g src unvisited h = foldr H.insert h [WE src w d|(d,w)<-successors g src,elem d unvisited]
...
primAux g rs st h
	|H.isEmpty h = (st,rs)
	|not $ elem b rs = primAux g rs st (H.delMin h)
	|otherwise = primAux g res (x:st) (addEdges g b res (H.delMin h))
		where
		x@(WE a w b) = H.minElem h
		res = (delete b rs)
-- primAux _ _ _ _ = undefined
-----------------------------------------------------------------------------
...
minimumSpanningForest g = (map (prim g) (vertices g))
