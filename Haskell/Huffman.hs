...
weights xs = aux xs D.empty
	where
		aux xs dict | (length xs /= 0) = D.insert (head xs) (length $ filter (head xs ==) xs) (aux (tail xs) dict) 
					|otherwise = dict
{-
...
huffmanLeaves xs = aux (D.keysAndValues (weights(xs))) 
	where
		aux xs 	
			| (length xs) == 0 = PQ.empty	
			| otherwise = PQ.enqueue (WLeaf a b) (aux (tail xs))
				where (a,b)=head xs
...
huffmanTree xs 
	| (PQ.isEmpty $ PQ.dequeue leaves) = error "HuffmanTree: the string must have at least two different symbols"
	|otherwise = PQ.first $ aux leaves
	where 
		leaves = huffmanLeaves xs
		aux queue 
			| (PQ.isEmpty $ PQ.dequeue queue) = queue
			| otherwise = aux (PQ.enqueue (merge (PQ.first queue) (PQ.first $ PQ.dequeue queue)) (PQ.dequeue $ PQ.dequeue queue))
...
joinDics dic1 dic2 
	|D.isEmpty dic2 = dic1
	|D.isEmpty dic1 = dic2
	|(dis (D.keys dic1) (D.keys dic2)) = joinDics (D.insert a b dic1) (D.delete a dic2)
	|otherwise = error "Diccionarios no disjuntos"
		where 
			(a,b)=head $ D.keysAndValues dic2
			dis xs ys 
				| xs == [] = True
				| elem (head xs) ys = False
				| otherwise = dis (tail xs) ys 
{-
...
prefixWith n dict = aux n (D.keysAndValues dict)
	where
		aux n xs 
			|(length xs) == 0 = dict
			|otherwise = D.updateOrInsert a (n:) b (aux n (tail xs))
				where 
					(a,b)=head xs
{-
...
huffmanCode xs = undefined
-- huffmanCode xs = aux (huffmanTree xs) D.empty []
	-- where 
		-- aux tree dic ys
			-- | (D.keys dic)==(D.keys (weights xs)) = dic
			-- |(esta (huffmanLeaves xs) a) = prefixWith 0 (joinDics (dicl c ys) dic)
			-- |(esta (huffmanLeaves xs) b) = prefixWith 1 (joinDics (dicl d ys) dic)
			-- | not (esta (huffmanLeaves xs) a) = aux a dic (0:ys)
			-- | otherwise = aux b dic (1:ys)
			-- where
				-- (WNode a b n1) = tree
				-- (WLeaf c n2) = a
				-- (WLeaf d n3) = b
				-- dicl l zs =  D.insert l zs D.empty
				-- esta queue leave 
					-- | (PQ.isEmpty queue) = False
					-- | (PQ.first queue) == leave = True
					-- | otherwise = esta (PQ.dequeue queue) leave
{-
