...
aux g dict stack
	...
	| not(colored ver) = aux g (D.insert ver color dict) (pushAll (S.pop stack)[(x,nextColor color)|x<-(successors g ver)])
	|((sameColor ver) == (color)) = aux g dict (S.pop stack)
	| otherwise = Nothing
-- 
...
--
  where
	...
	sameColor v = D.valueOf' v dict
	(ver,color) = S.top stack
--
...
g2  = mkGraphEdges vertices edges 
  where 
        vertices =  [A .. F]
        edges = [(A,B),(B,C),(B,D),(D,E),(D,F),(C,E),(E,G)]
-- *BiPartite> biColored g2
-- Just Dictionary(A->Red,B->Blue,C->Red,D->Red,E->Blue,G->Red,F->Blue)
